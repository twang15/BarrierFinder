...
struct global_memory {
  ...
  pthread_mutex_t (section_lock)[(64)];
  barrier_t (barrier_rank);
  barrier_t (barrier_key);
  double *ranktime;
  double *sorttime;
  double *totaltime;
  int final;
  ...
  unsigned int rs;
  unsigned int rf;
  struct prefix_node prefix_tree[2 * 64];
} *global;
...
# 260 "radix.c"
  if((key[0] == ((void*)0)) || (key[1] == ((void*)0))
      || (key[0] == ((void*)0)) || (key[1] == ((void*)0))
      || (key_partition == ((void*)0)) || (rank_partition == ((void*)0))
      || (rank_me == ((void*)0))
    )
  {
  ...
  for (i=0;i<number_of_processors;i++) {
    gp[i].rank_ff = (int *) malloc(radix*sizeof(int)+4096);;
  }
  {pthread_mutex_init(&(global->lock_Index), ((void*)0));}
  {pthread_mutex_init(&(global->rank_lock), ((void*)0));}
  {int i; for (i = 0; i < (64); i++)
    pthread_mutex_init(&((global->section_lock)[i]), ((void*)0));}
  {
    (global->barrier_rank).gsense=1;
    (global->barrier_rank).count=0;
  }
  {
...
# 376 "radix.c"
  for (i = 1; i < number_of_processors; i++) {
    {
      pthread_t t;
      pthread_create(&t, ((void*)0), (slave_sort), ((void*)0));
    }
  }
...
int atomic_cmpxchg(int oldcount, int newcount)
{
  ...
  {
    updatedcount = (global->barrier_key).count;
...
int atomic_cmpxchg1(int oldcount, int newcount)
{
  ...
  int temp, updatedcount;
  temp = (global->barrier_rank).count;

  if(oldcount == temp)
  {
    (global->barrier_rank).count = newcount;

    updatedcount = oldcount;
  }
  else
  {
    updatedcount = (global->barrier_rank).count;
...
void slave_sort()
{
  ...
  for (loopnum=0;loopnum<max_num_digits;loopnum++) {
...

    {
      ...
      int lsense = (global->barrier_rank).gsense;
      while (1) {
        int oldcount = (global->barrier_rank).count;
        int newcount = oldcount + lsense;
        int updatedcount, retVal;
        ...
        if (updatedcount == oldcount) {
          if ((newcount == (number_of_processors) && lsense == 1) || (newcount == 0 && lsense == -1))
          {
            (global->barrier_rank).gsense = -lsense;
            Criteria(3);
          }
          break;
        }
      }
      while ((global->barrier_rank).gsense == lsense)
        ;
      Criteria(3);
    ...
    if ((MyNum & 0x1) == 0) {
      ...
      {
        while ((global->prefix_tree[base + (offset >> 1)].done) == 1);
        {(global->prefix_tree[base + (offset >> 1)].done) = 1;}
      }
      ;
    }
    while ((offset & 0x1) != 0) {
...

{
  ...
  a1 = (double)((int)(t1 / 8388608.0e0));
  a2 = t1 - a1 * 8388608.0e0;
  b1 = (double)((int)(t2 / 8388608.0e0));
  b2 = t2 - b1 * 8388608.0e0;
  t1 = a1 * b2 + a2 * b1;
  t2 = (double)((int)(t1 / 8388608.0e0));
  t2 = t1 - t2 * 8388608.0e0;
  t1 = t2 * 8388608.0e0 + a2 * b2;
  t2 = (double)((int)(t1 / 70368744177664.0e0));

  return (t1 - t2 * 70368744177664.0e0);
}

double ran_num_init(k, b, t)
...

{
  ...
  ran_num = ran_num_init((key_start << 2) + 1, 314159265.0e0, 1220703125.0e0);
  sum = ran_num / 70368744177664.0e0;
  key_from = (int *) key[from];
  for (i = key_start; i < key_stop; i++) {
    ran_num = product_mod_46(ran_num, 1220703125.0e0);
    sum = sum + ran_num / 70368744177664.0e0;
    ran_num = product_mod_46(ran_num, 1220703125.0e0);
    sum = sum + ran_num / 70368744177664.0e0;
    ran_num = product_mod_46(ran_num, 1220703125.0e0);
    sum = sum + ran_num / 70368744177664.0e0;
