...

{
  ...
  {int i; for (i = 0; i < (MAX_PROCESSORS); i++)
    pthread_mutex_init(&((global->section_lock)[i]), NULL);}
  {
...
int atomic_cmpxchg1(int oldcount, int newcount)
{
  ...
  {
    updatedcount = (global->barrier_rank).count; // updatedcount != oldcount, update fails.
    //noScheduling(); 
  }
  return updatedcount;
}
