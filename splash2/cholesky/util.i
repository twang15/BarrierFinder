...
SMatrix NewMatrix(n, m, nz)
{
  ...
  M.n = n; M.m = m;
  M.col = (int *) MyMalloc((n+1)*sizeof(int), 888);
  M.startrow = (int *) MyMalloc((n+1)*sizeof(int), 888);
  M.row = (int *) MyMalloc((m+n)*sizeof(int), 888);
  if (nz) {
    M.nz = (double *) MyMalloc((m+n)*sizeof(double), 888);
  }
  else M.nz = ((void*)0);
...
double *NewVector(n)
{
  ...
  v = (double *) MyMalloc(n*sizeof(double), 888);
...
int *where;
{
  ...
  while (i < n) {
    ...
    for (j=0; j<perline && i<n; j++) {
      tmp = buf[(j+1)*persize]; buf[(j+1)*persize] = 0;
...
SMatrix L;
{
 ...
 { int s, n; s = L.row[first[j]]; n = j; link[n] = link[s]; link[s] = n; };
      j = nextj;
    }
 ...
 fprintf(stderr, "%d ", L.row[j]);
    ...
    if (first[i] < L.col[i+1]) {
      { int s, n; s = L.row[first[i]]; n = i; link[n] = link[s]; link[s] = n; };
    }
  }
