...
double *ops, *misses, *runtime;
{
  ...
  *runtime += (this_ops+(this_misses)*4.74)/(1.0+4.74/BS);
}
...
double *misses, *runtime;
{
  ...
  *runtime += (0 +(this_misses)*4.74)/(1.0+4.74/BS);
}
...
EmbedBlocks(P)
{
 ...
 (LB.entry[block].block)->owner = EmbeddedOwner(block);
      }
    }
