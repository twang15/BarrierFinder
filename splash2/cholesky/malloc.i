...
InitOneFreeList(p)
{
  ...
  {pthread_mutex_init(&(mem_pool[p].memoryLock), ((void*)0));};
  if (p > 0) {
    mem_pool[p].freeBlock = (unsigned **)
      malloc((16 +1)*sizeof(unsigned *));;
    MigrateMem(mem_pool[p].freeBlock, (16 +1)*sizeof(unsigned *), p);
  }
  else {
    mem_pool[p].freeBlock = (unsigned **)
      malloc((16 +1)*sizeof(unsigned *));;
    MigrateMem(mem_pool[p].freeBlock, (16 +1)*sizeof(unsigned *),
        888);
  }
  for (j=0; j<=16; j++)
    mem_pool[p].freeBlock[j] = ((void*)0);
...
int size, home;
{
  ...
  if (bucket < 16) {
    if (mem_pool[home].freeBlock[bucket]) {
      ...
      if (result)
 mem_pool[home].freeBlock[bucket] = (*((unsigned **) result));
 ...
 if (leftover > (1<<(16 -1))) {
   ...
   if (prev)
     (*((unsigned **) prev)) = (*((unsigned **) d));
   else
     mem_pool[home].freeBlock[16] = (*((unsigned **) d));
  ...
  if (result) {
    ...
    block_size = ((alloc_size) > (4*(1<<16)) ? (alloc_size) : (4*(1<<16)));
    ...
    if (block_size == alloc_size)
      ...
      mem_pool[home].tally += (freespace[-1]);
...
unsigned *block;
{
  ...
  (*((unsigned **) block)) = (unsigned *) mem_pool[home].freeBlock[bucket];
