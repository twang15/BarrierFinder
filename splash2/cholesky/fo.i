...
PreAllocate1FO()
{
  ...
  DiagBlock = (struct BlockList ***) MyMalloc(P*sizeof(struct BlockList **),
            888);
  ToReceive = (int **) MyMalloc(P*sizeof(int *), 888);
  NReceived = (int **) MyMalloc(P*sizeof(int *), 888);
  for (i=0; i<P; i++) {
...
struct LocalCopies *lc;
{
 ...
 lc->max_panel = j*(j+1)/2;
  ...
  for (i=0; i<LB.n_partitions; i++)
    AllBlocks[MyNum][i] = DiagBlock[MyNum][i] = ((void*)0);
...
struct LocalCopies *lc;
{
  ...
  for (j=0; j<LB.n; j+=node[j])
    if (!LB.domain[j]) {
      some = 1;
      if ((LB.entry[LB.col[j]].block)->owner == MyNum &&
   (LB.entry[LB.col[j]].block)->remaining == 0)
...
struct LocalCopies *lc;
{
  ...
  else if (update != ((void*)0)) {
  }
  else {
    if (((LB.entry[src].block)->i) == ((LB.entry[src].block)->j))
      DiagReceived(src, MyNum, lc);
    else
      BlockReceived(src, MyNum, lc);

    NReceived[MyNum][LB.renumbering[((LB.entry[src].block)->j)]]--;
    if (NReceived[MyNum][LB.renumbering[((LB.entry[src].block)->j)]] == 0)
      FreeColumnListFO(MyNum, LB.renumbering[((LB.entry[src].block)->j)]);
  }
...
struct LocalCopies *lc;
{
  ...
  column = LB.renumbering[((LB.entry[diag].block)->j)];
  diagbl->next = ((void*)0);
  DiagBlock[MyNum][column] = diagbl;

  column = ((LB.entry[diag].block)->j);
  for (i=LB.col[column]+1; i<LB.col[column+1]; i++)
    if ((LB.entry[i].block)->owner == MyNum &&
 (LB.entry[i].block)->remaining == 0) {
      BDiv(diag, i, diagbl->length, (LB.entry[i].block)->length,
    diagbl->nz, (LB.entry[i].block)->nz, MyNum, lc);
      BlockDoneFO(i, MyNum, lc);
    }


  if (((LB.entry[diag].block)->j)+LB.partition_size[((LB.entry[diag].block)->j)] == LB.n &&
      ((LB.entry[diag].block)->owner) == MyNum)
    for (i=0; i<P; i++)
      Send(-1, -1, -1, -1, (struct Update *) ((void*)0), i, MyNum, lc);
}
...
struct LocalCopies *lc;
{
  ...
  column = LB.renumbering[((LB.entry[block].block)->j)];
...
struct LocalCopies *lc;
{
  ...
  bl->theBlock = block;
  bl->row = ((LB.entry[block].block)->i); bl->col = ((LB.entry[block].block)->j);
  bl->length = (LB.entry[block].block)->length;

    bl->structure = (LB.entry[block].block)->structure;
    bl->nz = (LB.entry[block].block)->nz;
...
struct LocalCopies *lc;
{
  (LB.entry[dest_block].block)->remaining--;
  if ((LB.entry[dest_block].block)->remaining == 0)
    BlockReadyFO(dest_block, MyNum, lc);
  else if ((LB.entry[dest_block].block)->remaining == -1)
    printf("*** Error rem ***\n");
}
...
struct LocalCopies *lc;
{
  ...
  else if ((LB.entry[dest_block].block)->owner != MyNum)
    return 0;

  if (is_diag) {

    if (!below_bl->structure)
      destination = (LB.entry[dest_block].block)->nz;
    else
   ...
   below_bl->nz, destination, MyNum, lc);
    ...
    if (destination == lc->updatetmp) {
      ...
      ScatterUpdateFO(below_bl->length, below_bl->structure,
        ...
        (LB.entry[dest_block].block)->length,
        lc->updatetmp, (LB.entry[dest_block].block)->nz);
    }
  }
    ...
    if (below_bl->length == (LB.entry[dest_block].block)->length)
      relative_i = ((void*)0);
    else if (!(LB.entry[dest_block].block)->structure)
      relative_i = below_bl->structure;
    else {
      FindRelativeIndices(below_bl->structure, below_bl->length,
     (LB.entry[dest_block].block)->structure,
     (LB.entry[dest_block].block)->length, lc->relative);
...
struct LocalCopies *lc;
{
  ...
  for (bi=LB.col[LB.n+which_domain]; bi<LB.col[LB.n+which_domain+1]; bi++) {
    for (bj=LB.col[LB.n+which_domain]; bj<=bi; bj++) {
      desti = ((LB.entry[bi].block)->i);
      destj = ((LB.entry[bj].block)->i);

      dest_block = FindBlock(desti, destj);
      Send(which_domain, dest_block, bi, bj, (struct Update *) -19,
    ((LB.entry[dest_block].block)->owner), MyNum, lc);
    }
  }
}
...
struct LocalCopies *lc;
{
  ...
  desti = ((LB.entry[bli].block)->i); destj = ((LB.entry[blj].block)->i);
  dest_block = FindBlock(desti, destj);
  ...
  if ((LB.entry[bli].block)->structure && (LB.entry[dest_block].block)->structure) {
    if ((LB.entry[bli].block)->length != (LB.entry[dest_block].block)->length) {
      FindRelativeIndices((LB.entry[bli].block)->structure, (LB.entry[bli].block)->length,
     (LB.entry[dest_block].block)->structure,
     (LB.entry[dest_block].block)->length, lc->relative);
      relative_i = lc->relative;
    }
  ...
  else if ((LB.entry[bli].block)->structure)
    relative_i = (LB.entry[bli].block)->structure;
  else
    relative_i = ((void*)0);

  if ((LB.entry[blj].block)->structure)
    relative_j = (LB.entry[blj].block)->structure;
  else
    relative_j = ((void*)0);

  ScatterUpdateFO2((LB.entry[bli].block)->length, relative_i,
    (LB.entry[blj].block)->length, relative_j,
    stride, (LB.entry[dest_block].block)->length,
    update, (LB.entry[dest_block].block)->nz);
...
struct LocalCopies *lc;
{
  ...
  if (((LB.entry[block].block)->i) == ((LB.entry[block].block)->j)) {
    BFac(block, MyNum, lc);
    ...
    if (DiagBlock[MyNum][column]) {
      ...
      BDiv(LB.col[((LB.entry[block].block)->j)], block, diagbl->length, (LB.entry[block].block)->length,
    diagbl->nz, (LB.entry[block].block)->nz, MyNum, lc);
      BlockDoneFO(block, MyNum, lc);
    }
...
struct LocalCopies *lc;
{
  ...
  if (scatter_decomposition) {
    P_row = LB.mapI[LB.renumbering[((LB.entry[block].block)->i)]]%P_dimi;
    P_col = LB.mapJ[LB.renumbering[((LB.entry[block].block)->i)]]%P_dimj;


    for (i=0; i<P_dimj; i++)
      Send(block, block, 0, 0, (struct Update *) ((void*)0), P_row + i*P_dimi, MyNum, lc);


    for (i=0; i<P_dimi; i++)
      if (i != P_row)
 Send(block, block, 0, 0, (struct Update *) ((void*)0), i + P_col*P_dimi, MyNum, lc);
  }
  else {
    for (i=0; i<P; i++)
      Send(block, block, 0, 0, (struct Update *) ((void*)0), i, MyNum, lc);
  }
}
...
ComputeRemainingFO()
{
 ...
 destj = ((LB.entry[j].block)->i);
 desti = ((LB.entry[i].block)->i);
 dest_block = FindBlock(desti, destj);
 (LB.entry[dest_block].block)->nmod++;
      }
      desti = ((LB.entry[i].block)->i);
...
struct LocalCopies *lc;
{
 ...
 if (MyNum == -1 || (LB.entry[i].block)->owner == MyNum)
   (LB.entry[i].block)->remaining = (LB.entry[i].block)->nmod;
}
...
ComputeReceivedFO()
{
 ...
 if (scatter_decomposition) {
   ...
   for (i=0; i<P_dimj; i++) {
     ...
     ToReceive[destp][LB.renumbering[((LB.entry[block].block)->j)]]++;
   }
   ...
   for (i=0; i<P; i++)
     ToReceive[i][LB.renumbering[((LB.entry[block].block)->j)]]++;
 }
      }
    }
}
