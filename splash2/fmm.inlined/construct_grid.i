...
# 24 "construct_grid.c" 2
# 1 "/usr/include/values.h" 1 3 4
# 27 "/usr/include/values.h" 3 4
# 1 "/home/tao/tools/llvm.optO2/bin/../lib/clang/3.7.0/include/limits.h" 1 3 4
# 37 "/home/tao/tools/llvm.optO2/bin/../lib/clang/3.7.0/include/limits.h" 3 4
...
# 38 "/home/tao/tools/llvm.optO2/bin/../lib/clang/3.7.0/include/limits.h" 2 3 4
# 28 "/usr/include/values.h" 2 3 4
# 51 "/usr/include/values.h" 3 4
# 1 "/home/tao/tools/llvm.optO2/bin/../lib/clang/3.7.0/include/float.h" 1 3 4
# 52 "/usr/include/values.h" 2 3 4
# 25 "construct_grid.c" 2
# 1 "./defs.h" 1
# 27 "./defs.h"
...
# 30 "construct_grid.c" 2
# 1 "./construct_grid.h" 1
# 28 "./construct_grid.h"
...
extern void ConstructGridLists(int my_id, box *b);
extern void ConstructInteractionLists(int my_id, box *b);
extern void MergeLocalGridSize_1(int my_id);
extern void MergeLocalGridSize_2(int my_id);
extern void DetermineLocalGridSize(int my_id);
# 31 "construct_grid.c" 2
...
void SplitParticles(int my_id, particle **p_array, int length,
      particle **p_dist, int num_p_dist[4], box *pb);
...
ConstructGrid (int my_id, time_info *local_time, int time_all)
{
   ...
   DetermineGridSize(my_id);
   FreeBoxes(my_id);
   InitPartition(my_id);
   if (time_all)
      {long time(); (start) = time(0);};
   if ((Local[my_id].Num_Particles) > 0) {
      ConstructLocalGrid(my_id);

      MergeLocalGrid(my_id);
 ...
 asm("lock cmpxchg %1,%2" :
     "=a" (updatedcount) :
     "r" (newcount), "m" ((G_Memory->synch).count), "0a" (oldcount) );
...
;
   ...
   if (time_all) {
      local_time[(Local[my_id].Time_Step)].other_time = start - init;
      local_time[(Local[my_id].Time_Step)].construct_time = finish - start;
   }
}
...
;
   ...
   if (time_all) {
      local_time[(Local[my_id].Time_Step)].list_time = finish - start;
   }
}
...
DestroyGrid (int my_id, time_info *local_time, int time_all)
{
  ...
  if ((Local[my_id].Max_Particles) <= (Local[my_id].Num_Particles)) {
  ...
  (Local[my_id].Particles)[(Local[my_id].Num_Particles)++] = p;
      }
   }
   if (my_id == 0)
      Grid = ((void*)0);
   if (time_all) {
      {long time(); (finish) = time(0);};
      local_time[(Local[my_id].Time_Step)].other_time += finish - start;
   }
}
# 218 "construct_grid.c"
void
PrintGrid (int my_id)
{
   if (Grid != ((void*)0)) {
...
{
   DetermineLocalGridSize(my_id);

   MergeLocalGridSize(my_id);
...
DetermineLocalGridSize (int my_id)
{
   ...
   Local[my_id].Local_X_Max = -2.221e308;
   Local[my_id].Local_X_Min = 2.221e308;
   Local[my_id].Local_Y_Max = -2.221e308;
   Local[my_id].Local_Y_Min = 2.221e308;
   for (i = 0; i < (Local[my_id].Num_Particles) - 1; i += 2) {
      x_pos1 = (Local[my_id].Particles)[i]->pos.x;
      y_pos1 = (Local[my_id].Particles)[i]->pos.y;
      x_pos2 = (Local[my_id].Particles)[i + 1]->pos.x;
      y_pos2 = (Local[my_id].Particles)[i + 1]->pos.y;
      if (x_pos1 > x_pos2) {
  ...
  Local[my_id].Local_Y_Min = y_min_challenger;
   ...
   if (i == ((Local[my_id].Num_Particles) - 1)) {
      x_max_challenger = (Local[my_id].Particles)[i]->pos.x;
      x_min_challenger = (Local[my_id].Particles)[i]->pos.x;
      y_max_challenger = (Local[my_id].Particles)[i]->pos.y;
      y_min_challenger = (Local[my_id].Particles)[i]->pos.y;
      if (x_max_challenger > Local[my_id].Local_X_Max)
  Local[my_id].Local_X_Max = x_max_challenger;
      if (x_min_challenger < Local[my_id].Local_X_Min)
  Local[my_id].Local_X_Min = x_min_challenger;
...
void MergeLocalGridSize_1(int my_id)
{
...
void MergeLocalGridSize_2(int my_id)
{
   real *my_f_array, *their_f_array;
   real x_max_challenger, x_min_challenger, y_max_challenger, y_min_challenger;
   int i;
...
{
   Local[my_id].Local_Grid = InitGrid(my_id);
   InsertParticlesInTree(my_id, (Local[my_id].Particles), (Local[my_id].Num_Particles),
...
InitGrid (int my_id)
{
  ...
  Local[my_id].Local_Y_Min = ldexp(1.0, exp - 1);
   ...
   ret_box = InitBox(my_id, grid_x_center, grid_y_center, grid_length, ((void*)0));
   return ret_box;
}
# 548 "construct_grid.c"
void
InsertParticlesInTree (int my_id, particle **p_list, int num_of_particles, box *root)
{
   ...
   for (i = 0; i < num_of_particles; i++) {
      ...
      while (dest_box->num_particles > 40) {
  SubdivideBox(my_id, dest_box);
  if (dest_box->num_children == 1) {
     for (j = 0; dest_box->children[j] == ((void*)0); j++)
        ;
...
FindHome (int my_id, particle *p, box *current_home)
{
  ...
  if (p->pos.x > pb->x_center) {
     if (pb->children[0] == ((void*)0))
        CreateChild(my_id, pb, 0);
     pb = pb->children[0];
  }
  else {
     if (pb->children[1] == ((void*)0))
  ...
  if (p->pos.x > pb->x_center) {
     if (pb->children[3] == ((void*)0))
        CreateChild(my_id, pb, 3);
     pb = pb->children[3];
  }
  else {
     if (pb->children[2] == ((void*)0))
...
CreateChild (int my_id, box *pb, int new_child_num)
{
   ...
   child_length = pb->length / (real) 2;
   child_offset = pb->length / (real) 4;
   if (new_child_num == 0) {
...
{
   MLGHelper(my_id, Local[my_id].Local_Grid, Grid, ((void*)0));
}
...
MLGHelper (int my_id, box *local_box, box *global_box, box *global_parent)
{
  ...
  if (global_box == ((void*)0)) {
     ...
     if (global_box->type == PARENT) {
        success = TRUE;
        for (i = 0; i < 4; i++) {
    if (local_box->children[i] != ((void*)0))
  ...
  int num_p_dist[4], box *pb)
{
  ...
  if (p->pos.x > pb->x_center)
     ...
     *(p_dist + 40 + num_p_dist[1]++) = p;
      }
      else {
  if (p->pos.x > pb->x_center)
     *(p_dist + (3 * 40) + num_p_dist[3]++) = p;
  else
     *(p_dist + (2 * 40) + num_p_dist[2]++) = p;
      }
   }
}
...
InsertParticlesInLeaf (int my_id, particle **p_array, int length, box *b)
{
   ...
   if ((length + b->num_particles) > 40) {
      for (i = b->num_particles, j = length - 1; i < 40;
    i++, j--)
...
InsertBoxInGrid (int my_id, box *b, box *pb)
{
  ...
  success = FALSE;
      ...
      if (pb->children[b->child_num] == ((void*)0)) {
...
RemoveBoxFromGrid (int my_id, box *b, box *pb)
{
  ...
  pb->children[b->child_num] = ((void*)0);
  b->parent = ((void*)0);
  pb->num_children -= 1;
  success = TRUE;
      }
...
CleanupGrid (int my_id)
{
   ...
   b_scan = Local[my_id].Childless_Partition;
   while (b_scan != ((void*)0)) {
      if (((b_scan->parent != ((void*)0)) || (b_scan == Grid))
...
SetColleagues (int my_id, box *b)
{
  ...
  for (j = 0; j < 4; j++) {
   ...
   if (b->type == PARENT) {
      for (i = 0; i < 4; i++) {
  if (b->children[i] != ((void*)0)) {
     b->children[i]->construct_synch = 1;
  }
      }
   }
}
# 1115 "construct_grid.c"
void
