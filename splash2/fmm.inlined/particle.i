...
CreateDistribution (cluster_type cluster, model_type model)
{
   ...
   r_scale = 3 * 3.14159265358979323846 / 16;
   v_scale = (real) sqrt(1.0 / (double) r_scale);
   r_sum.x = (real) 0.0;
   r_sum.y = (real) 0.0;
   v_sum.x = (real) 0.0;
   v_sum.y = (real) 0.0;
   initstate(0, particle_state, 256);
  ...
  do {
     ...
     temp_r = (((new_particle->pos).x * (new_particle->pos).x) + ((new_particle->pos).y * (new_particle->pos).y));
  }
  ...
  break;
       case PLUMMER:
  do
     radius = (real) 1.0 / (real) sqrt(pow(XRand(0.0, 0.999),
        -2.0/3.0) - 1);
  while (radius > 9.0);
  PickShell(&(new_particle->pos), r_scale * radius);
  break;
      }
      { r_sum.x = r_sum.x + (new_particle->pos).x; r_sum.y = r_sum.y + (new_particle->pos).y; };
  ...
  y_vel = XRand(0.0, 0.1);
      ...
      { v_sum.x = v_sum.x + (new_particle->vel).x; v_sum.y = v_sum.y + (new_particle->vel).y; };
   }
  ...
  { v_sum.x = v_sum.x + (new_particle->vel).x; v_sum.y = v_sum.y + (new_particle->vel).y; };
   ...
   { r_sum.x = r_sum.x / (real) Total_Particles; r_sum.y = r_sum.y / (real) Total_Particles; };
   { v_sum.x = v_sum.x / (real) Total_Particles; v_sum.y = v_sum.y / (real) Total_Particles; };
   for (i = 0; i < Total_Particles; i++) {
      new_particle = Particle_List[i];
      { (new_particle->pos).x = (new_particle->pos).x - r_sum.x; (new_particle->pos).y = (new_particle->pos).y - r_sum.y; };
      { (new_particle->vel).x = (new_particle->vel).x - v_sum.x; (new_particle->vel).y = (new_particle->vel).y - v_sum.y; };
   }
}
...
PickShell (vector *v, real radius)
{
   ...
   do {
      v->x = XRand(-1.0, 1.0);
      v->y = XRand(-1.0, 1.0);
      temp_r = (((*v).x * (*v).x) + ((*v).y * (*v).y));
   }
   while (temp_r >1.0);
   r_scale = radius / (real) sqrt(temp_r);
   { (*v).x = (*v).x * r_scale; (*v).y = (*v).y * r_scale; };
}
