...
int atomic_cmpxchg(int oldcount, int newcount)
{
  ...
  temp = (G_Memory->synch).count;

  if(oldcount == temp)
  {
    (G_Memory->synch).count = newcount;

    updatedcount = oldcount;
  }
  else
  {
    updatedcount = (G_Memory->synch).count;
...
main (int argc, char *argv[])
{
   ...
   for (i = 1; i < Number_Of_Processors; i++) {
      {
pthread_t t;
pthread_create(&t, ((void*)0), (ParallelExecute), ((void*)0));
};
   }
   ParallelExecute();
   {;};
...
ParallelExecute ()
{
   ...
   local_time = (time_info *) malloc(sizeof(struct _Time_Info) * 10);
   {
   ...
   if (my_id == 0) {
      ...
      CreateParticleList(my_id, ((Total_Particles * 4.0)
     / Number_Of_Processors));

   }
   num_boxes = 1.333 * (Total_Particles / (((40 > 5) ? .375 : .750) * 40));
   if (my_id == 0)
      CreateBoxes(my_id, 1.5 * num_boxes);
   else
      CreateBoxes(my_id, 1.5 * num_boxes * (((Total_Particles/Number_Of_Processors) > 128) ? 4.0 : 8.0) / Number_Of_Processors);
  ...
  ;
   ...
   for ((Local[my_id].Time_Step) = 0; (Local[my_id].Time_Step) < Time_Steps; (Local[my_id].Time_Step)++) {

      if ((Local[my_id].Time_Step) == 2) {
...

      {
        ...
        if (time_all) {
          local_time[(Local[my_id].Time_Step)].partition_time = finish - start;
        }
      }
...

      {
        ...
        if (time_all) {
          local_time[(Local[my_id].Time_Step)].pass_time = upward_end - start;
          local_time[(Local[my_id].Time_Step)].inter_time = interaction_end - upward_end;
          local_time[(Local[my_id].Time_Step)].barrier_time = barrier_end - interaction_end;
          local_time[(Local[my_id].Time_Step)].pass_time += downward_end - barrier_end;
          local_time[(Local[my_id].Time_Step)].intra_time = finish - downward_end;
        }
      }
      ...
      (Local[my_id].Timing)[(Local[my_id].Time_Step)].total_time = finish - start;
  ...
  ;
   for ((Local[my_id].Time_Step) = 0; (Local[my_id].Time_Step) < Time_Steps; (Local[my_id].Time_Step)++) {
     timing = &((Local[my_id].Timing)[(Local[my_id].Time_Step)]);
     timing->other_time = local_time[(Local[my_id].Time_Step)].other_time;
     timing->construct_time = local_time[(Local[my_id].Time_Step)].construct_time;
     timing->list_time = local_time[(Local[my_id].Time_Step)].list_time;
     timing->partition_time = local_time[(Local[my_id].Time_Step)].partition_time;
     timing->pass_time = local_time[(Local[my_id].Time_Step)].pass_time;
     timing->inter_time = local_time[(Local[my_id].Time_Step)].inter_time;
     timing->barrier_time = local_time[(Local[my_id].Time_Step)].barrier_time;
     timing->intra_time = local_time[(Local[my_id].Time_Step)].intra_time;
   }
