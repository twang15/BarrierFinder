...
InitPartition (int my_id)
{
   ...
   Local[my_id].Childless_Partition = ((void*)0);
   for (i = 0; i < 100; i++) {
      Local[my_id].Parent_Partition[i] = ((void*)0);
   }
...
InsertBoxInPartition (int my_id, box *b)
{
   ...
   if (b->type == CHILDLESS) {
      b->prev = ((void*)0);
      if (Local[my_id].Childless_Partition != ((void*)0))
...
RemoveBoxFromPartition (int my_id, box *b)
{
  ...
  b->next->prev = b->prev;
      if ((b->level == Local[my_id].Max_Parent_Level) &&
   (Local[my_id].Parent_Partition[b->level] == ((void*)0))) {
...
ComputeCostOfBox (int my_id, box *b)
{
   ...
   int j;
   int new_cost;
   cost_info cost_list[40];
   cost_info winner;
   int winner_index;
   int cost_index[40];

   if (b->type == PARENT)
      b->cost = ((b->num_v_list * (1.08 * ((35.9 * Expansion_Terms * Expansion_Terms) + (133.6 * Expansion_Terms))))
   / ((Expansion_Terms <= 20) ? 1 : ((Expansion_Terms - 20) * 50))) + 1;
   else {
      different_costs = 0;
  ...
  winner.cost /= winner.num;
      ...
      b->cost = (winner.cost * b->num_particles) / ((Expansion_Terms <= 20) ? 1 : ((Expansion_Terms - 20) * 50));
      if (b->cost == 0)
  b->cost = 1;
   }
}
...
CheckBox (int my_id, box *b, int partition_level)
{
  ...
  num_errors += 1;
      ...
      if (b->particles[b->num_particles - 1] == ((void*)0)) {
  {pthread_mutex_lock(&(G_Memory->io_lock));};
  ...
  num_errors += 1;
      ...
      if (b->particles[b->num_particles] != ((void*)0)) {
  {pthread_mutex_lock(&(G_Memory->io_lock));};
