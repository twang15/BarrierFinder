...
# 1 "/home/tao/tools/llvm.optO2/bin/../lib/clang/3.7.0/include/stddef.h" 1 3 4
# 90 "/home/tao/tools/llvm.optO2/bin/../lib/clang/3.7.0/include/stddef.h" 3 4
typedef int wchar_t;
...
# 1 "/home/tao/tools/llvm.optO2/bin/../lib/clang/3.7.0/include/limits.h" 1 3
# 37 "/home/tao/tools/llvm.optO2/bin/../lib/clang/3.7.0/include/limits.h" 3
...
# 38 "/home/tao/tools/llvm.optO2/bin/../lib/clang/3.7.0/include/limits.h" 2 3
# 31 "./defs.h" 2
# 143 "./defs.h"
...
struct _Box
{
  ...
  box *w_list[30];
  int num_w_list;
  complex mp_expansion[40];
  complex local_expansion[40];
  complex x_expansion[40];
  int exp_lock_index;
...
struct _G_Mem
{
   ...
   int count;
   pthread_mutex_t (lock_array)[(2048)];
   barrier_t (synch);
...
struct _Local_Memory {
   ...
   int pad_end[(4096 / (sizeof(int)))];
};
extern local_memory Local[64];

extern void InitGlobalMemory();
# 27 "interactions.c" 2


# 1 "./partition_grid.h" 1
# 30 "./partition_grid.h"
...
static real Inv[40 + 1];
static real OverInc[40 + 1];
static real C[2 * 40][2 * 40];
static complex One;
static complex Zero;
...
ComputeParticlePositions (int my_id, box *b)
{
   ...
   for (i = 0; i < b->num_particles; i++) {
      ...
      { new_acc.x = force.x / p->mass; new_acc.y = force.y / p->mass; };
      if (Local[my_id].Time_Step != 0) {
  { delta_acc.x = new_acc.x - (p->acc).x; delta_acc.y = new_acc.y - (p->acc).y; };
  { delta_vel.x = delta_acc.x * ((real) Timestep_Dur) / (real) 2.0; delta_vel.y = delta_acc.y * ((real) Timestep_Dur) / (real) 2.0; };
  { (p->vel).x = (p->vel).x + delta_vel.x; (p->vel).y = (p->vel).y + delta_vel.y; };
      }
      p->acc.x = new_acc.x;
      p->acc.y = new_acc.y;
      { delta_vel.x = (p->acc).x * ((real) Timestep_Dur) / (real) 2.0; delta_vel.y = (p->acc).y * ((real) Timestep_Dur) / (real) 2.0; };
      { avg_vel.x = (p->vel).x + delta_vel.x; avg_vel.y = (p->vel).y + delta_vel.y; };
      { delta_pos.x = avg_vel.x * (real) Timestep_Dur; delta_pos.y = avg_vel.y * (real) Timestep_Dur; };
      { (p->vel).x = avg_vel.x + delta_vel.x; (p->vel).y = avg_vel.y + delta_vel.y; };
      { (p->pos).x = (p->pos).x + delta_pos.x; (p->pos).y = (p->pos).y + delta_pos.y; };
   }
}
...
ComputeMPExp (int my_id, box *b)
{
   ...
   for (i = 0; i < b->num_particles; i++) {
      ...
      charge.r = p->charge;
      charge.i = (real) 0.0;
      { z0.r = particle_pos.r - box_pos.r; z0.i = particle_pos.i - box_pos.i; };
      z0_pow_n.r = One.r;
      z0_pow_n.i = One.i;
      for (j = 1; j < Expansion_Terms; j++) {
  { complex _c_temp; _c_temp.r = (z0_pow_n.r * charge.r) - (z0_pow_n.i * charge.i); _c_temp.i = (z0_pow_n.r * charge.i) + (z0_pow_n.i * charge.r); temp.r = _c_temp.r; temp.i = _c_temp.i; };
  { result_exp[j].r = result_exp[j].r + temp.r; result_exp[j].i = result_exp[j].i + temp.i; };
  { complex _c_temp; _c_temp.r = (z0_pow_n.r * z0.r) - (z0_pow_n.i * z0.i); _c_temp.i = (z0_pow_n.r * z0.i) + (z0_pow_n.i * z0.r); z0_pow_n.r = _c_temp.r; z0_pow_n.i = _c_temp.i; };
      }
   }
   {pthread_mutex_lock(&(G_Memory->lock_array)[(b->exp_lock_index)]);};
   for (i = 0; i < Expansion_Terms; i++) {
...
ShiftMPExp (int my_id, box *cb, box *pb)
{
   ...
   { z0.r = child_pos.r - parent_pos.r; z0.i = child_pos.i - parent_pos.i; };
   { real _denom; complex _c_temp; _denom = ((real) 1.0) / ((z0.r * z0.r) + (z0.i * z0.i)); _c_temp.r = ((One.r * z0.r) + (One.i * z0.i)) * _denom; _c_temp.i = ((One.i * z0.r) - (One.r * z0.i)) * _denom; z0_inv.r = _c_temp.r; z0_inv.i = _c_temp.i; };
   z0_pow_n.r = One.r;
   z0_pow_n.i = One.i;
   ...
   for (i = 1; i < Expansion_Terms; i++) {
      result_exp[i].r = (real) 0.0;
      result_exp[i].i = (real) 0.0;
      { complex _c_temp; _c_temp.r = (z0_pow_minus_n.r * z0_inv.r) - (z0_pow_minus_n.i * z0_inv.i); _c_temp.i = (z0_pow_minus_n.r * z0_inv.i) + (z0_pow_minus_n.i * z0_inv.r); z0_pow_minus_n.r = _c_temp.r; z0_pow_minus_n.i = _c_temp.i; };
      { complex _c_temp; _c_temp.r = (z0_pow_minus_n.r * cb->mp_expansion[i].r) - (z0_pow_minus_n.i * cb->mp_expansion[i].i); _c_temp.i = (z0_pow_minus_n.r * cb->mp_expansion[i].i) + (z0_pow_minus_n.i * cb->mp_expansion[i].r); temp_exp[i].r = _c_temp.r; temp_exp[i].i = _c_temp.i; };
      for (j = 1; j <= i; j++) {
  temp.r = C[i - 1][j - 1];
  temp.i = (real) 0.0;
  { complex _c_temp; _c_temp.r = (temp.r * temp_exp[j].r) - (temp.i * temp_exp[j].i); _c_temp.i = (temp.r * temp_exp[j].i) + (temp.i * temp_exp[j].r); temp.r = _c_temp.r; temp.i = _c_temp.i; };
  { result_exp[i].r = result_exp[i].r + temp.r; result_exp[i].i = result_exp[i].i + temp.i; };
      }
      temp.r = Inv[i];
      temp.i = (real) 0.0;
      { complex _c_temp; _c_temp.r = (temp.r * cb->mp_expansion[0].r) - (temp.i * cb->mp_expansion[0].i); _c_temp.i = (temp.r * cb->mp_expansion[0].i) + (temp.i * cb->mp_expansion[0].r); temp.r = _c_temp.r; temp.i = _c_temp.i; };
      { temp.r = result_exp[i].r - temp.r; temp.i = result_exp[i].i - temp.i; };
      { complex _c_temp; _c_temp.r = (z0_pow_n.r * z0.r) - (z0_pow_n.i * z0.i); _c_temp.i = (z0_pow_n.r * z0.i) + (z0_pow_n.i * z0.r); z0_pow_n.r = _c_temp.r; z0_pow_n.i = _c_temp.i; };
      { complex _c_temp; _c_temp.r = (temp.r * z0_pow_n.r) - (temp.i * z0_pow_n.i); _c_temp.i = (temp.r * z0_pow_n.i) + (temp.i * z0_pow_n.r); result_exp[i].r = _c_temp.r; result_exp[i].i = _c_temp.i; };
   }
   {pthread_mutex_lock(&(G_Memory->lock_array)[(pb->exp_lock_index)]);};
   for (i = 0; i < Expansion_Terms; i++) {
      { (pb->mp_expansion[i]).r = (pb->mp_expansion[i]).r + result_exp[i].r; (pb->mp_expansion[i]).i = (pb->mp_expansion[i]).i + result_exp[i].i; };
   }
...
UListInteraction (int my_id, box *source_box, box *dest_box)
{
  ...
  { complex _c_temp; _c_temp.r = (temp_vector.r * temp_charge.r) - (temp_vector.i * temp_charge.i); _c_temp.i = (temp_vector.r * temp_charge.i) + (temp_vector.i * temp_charge.r); temp_result.r = _c_temp.r; temp_result.i = _c_temp.i; };
  { result.r = result.r - temp_result.r; result.i = result.i - temp_result.i; };
      }
      result.i = -result.i;
      { (dest_box->particles[i]->field).r = (dest_box->particles[i]->field).r + result.r; (dest_box->particles[i]->field).i = (dest_box->particles[i]->field).i + result.i; };

   }

   dest_box->cost += (1.06 * 79.2 * source_box->num_particles * dest_box->num_particles);
...
VListInteraction (int my_id, box *source_box, box *dest_box)
{
   ...
   { z0.r = source_pos.r - dest_pos.r; z0.i = source_pos.i - dest_pos.i; };
   { real _denom; complex _c_temp; _denom = ((real) 1.0) / ((z0.r * z0.r) + (z0.i * z0.i)); _c_temp.r = ((One.r * z0.r) + (One.i * z0.i)) * _denom; _c_temp.i = ((One.i * z0.r) - (One.r * z0.i)) * _denom; z0_inv.r = _c_temp.r; z0_inv.i = _c_temp.i; };
   ...
   for (i = 1; i < Expansion_Terms; i++) {
      { complex _c_temp; _c_temp.r = (z0_pow_minus_n[i - 1].r * z0_inv.r) - (z0_pow_minus_n[i - 1].i * z0_inv.i); _c_temp.i = (z0_pow_minus_n[i - 1].r * z0_inv.i) + (z0_pow_minus_n[i - 1].i * z0_inv.r); z0_pow_minus_n[i].r = _c_temp.r; z0_pow_minus_n[i].i = _c_temp.i; };
      { complex _c_temp; _c_temp.r = (z0_pow_minus_n[i].r * source_box->mp_expansion[i].r) - (z0_pow_minus_n[i].i * source_box->mp_expansion[i].i); _c_temp.i = (z0_pow_minus_n[i].r * source_box->mp_expansion[i].i) + (z0_pow_minus_n[i].i * source_box->mp_expansion[i].r); temp_exp[i].r = _c_temp.r; temp_exp[i].i = _c_temp.i; };
   }
   for (i = 0; i < Expansion_Terms; i++) {
  ...
  { complex _c_temp; _c_temp.r = (temp.r * temp_exp[j].r) - (temp.i * temp_exp[j].i); _c_temp.i = (temp.r * temp_exp[j].i) + (temp.i * temp_exp[j].r); temp.r = _c_temp.r; temp.i = _c_temp.i; };
  if ((j & 0x1) == 0x0) {
     { result_exp.r = result_exp.r + temp.r; result_exp.i = result_exp.i + temp.i; };
  }
  else {
     { result_exp.r = result_exp.r - temp.r; result_exp.i = result_exp.i - temp.i; };
  }
      }
      { complex _c_temp; _c_temp.r = (result_exp.r * z0_pow_minus_n[i].r) - (result_exp.i * z0_pow_minus_n[i].i); _c_temp.i = (result_exp.r * z0_pow_minus_n[i].i) + (result_exp.i * z0_pow_minus_n[i].r); result_exp.r = _c_temp.r; result_exp.i = _c_temp.i; };
      if (i == 0) {
  temp.r = log(sqrt((double) ((z0.r * z0.r) + (z0.i * z0.i))));
  temp.i = (real) 0.0;
  { complex _c_temp; _c_temp.r = (temp.r * source_box->mp_expansion[0].r) - (temp.i * source_box->mp_expansion[0].i); _c_temp.i = (temp.r * source_box->mp_expansion[0].i) + (temp.i * source_box->mp_expansion[0].r); temp.r = _c_temp.r; temp.i = _c_temp.i; };
  { result_exp.r = result_exp.r + temp.r; result_exp.i = result_exp.i + temp.i; };
      }
      else {
  temp.r = Inv[i];
  temp.i = (real) 0.0;
  { complex _c_temp; _c_temp.r = (temp.r * z0_pow_minus_n[i].r) - (temp.i * z0_pow_minus_n[i].i); _c_temp.i = (temp.r * z0_pow_minus_n[i].i) + (temp.i * z0_pow_minus_n[i].r); temp.r = _c_temp.r; temp.i = _c_temp.i; };
  { complex _c_temp; _c_temp.r = (temp.r * source_box->mp_expansion[0].r) - (temp.i * source_box->mp_expansion[0].i); _c_temp.i = (temp.r * source_box->mp_expansion[0].i) + (temp.i * source_box->mp_expansion[0].r); temp.r = _c_temp.r; temp.i = _c_temp.i; };
  { result_exp.r = result_exp.r - temp.r; result_exp.i = result_exp.i - temp.i; };
      }
      { (dest_box->local_expansion[i]).r = (dest_box->local_expansion[i]).r + result_exp.r; (dest_box->local_expansion[i]).i = (dest_box->local_expansion[i]).i + result_exp.i; };

   }
   dest_box->cost += (1.08 * ((35.9 * Expansion_Terms * Expansion_Terms) + (133.6 * Expansion_Terms)));
}
...
WListInteraction (int my_id, box *source_box, box *dest_box)
{
   ...
   for (i = 0; i < dest_box->num_particles; i++) {
      ...
      { z0.r = particle_pos.r - source_pos.r; z0.i = particle_pos.i - source_pos.i; };
      { real _denom; complex _c_temp; _denom = ((real) 1.0) / ((z0.r * z0.r) + (z0.i * z0.i)); _c_temp.r = ((One.r * z0.r) + (One.i * z0.i)) * _denom; _c_temp.i = ((One.i * z0.r) - (One.r * z0.i)) * _denom; z0_inv.r = _c_temp.r; z0_inv.i = _c_temp.i; };
      for (j = Expansion_Terms - 1; j > 0; j--) {
  { result.r = result.r + (source_box->mp_expansion[j]).r; result.i = result.i + (source_box->mp_expansion[j]).i; };
  { complex _c_temp; _c_temp.r = (result.r * z0_inv.r) - (result.i * z0_inv.i); _c_temp.i = (result.r * z0_inv.i) + (result.i * z0_inv.r); result.r = _c_temp.r; result.i = _c_temp.i; };
      }
      { (dest_box->particles[i]->field).r = (dest_box->particles[i]->field).r + result.r; (dest_box->particles[i]->field).i = (dest_box->particles[i]->field).i + result.i; };

   }

   dest_box->cost += (1.11 * 29.2 * dest_box->num_particles * Expansion_Terms);
}
...
XListInteraction (int my_id, box *source_box, box *dest_box)
{
   ...
   for (i = 0; i < source_box->num_particles; i++) {
      ...
      for (j = 1; j < Expansion_Terms; j++) {
  { complex _c_temp; _c_temp.r = (z0_pow_minus_n.r * z0_inv.r) - (z0_pow_minus_n.i * z0_inv.i); _c_temp.i = (z0_pow_minus_n.r * z0_inv.i) + (z0_pow_minus_n.i * z0_inv.r); z0_pow_minus_n.r = _c_temp.r; z0_pow_minus_n.i = _c_temp.i; };
  { complex _c_temp; _c_temp.r = (charge.r * z0_pow_minus_n.r) - (charge.i * z0_pow_minus_n.i); _c_temp.i = (charge.r * z0_pow_minus_n.i) + (charge.i * z0_pow_minus_n.r); temp.r = _c_temp.r; temp.i = _c_temp.i; };
  { result_exp[j].r = result_exp[j].r + temp.r; result_exp[j].i = result_exp[j].i + temp.i; };
      }
   }
   {pthread_mutex_lock(&(G_Memory->lock_array)[(dest_box->exp_lock_index)]);};
   for (i = 0; i < Expansion_Terms; i++) {
      { (dest_box->x_expansion[i]).r = (dest_box->x_expansion[i]).r - result_exp[i].r; (dest_box->x_expansion[i]).i = (dest_box->x_expansion[i]).i - result_exp[i].i; };

   }
   {pthread_mutex_unlock(&(G_Memory->lock_array)[(dest_box->exp_lock_index)]);};
   source_box->cost += (1.15 * 56.0 * source_box->num_particles * Expansion_Terms);
}
...
ComputeSelfInteraction (int my_id, box *b)
{
  ...
  temp_charge.i = (real) 0.0;
  { complex _c_temp; _c_temp.r = (temp_vector.r * temp_charge.r) - (temp_vector.i * temp_charge.i); _c_temp.i = (temp_vector.r * temp_charge.i) + (temp_vector.i * temp_charge.r); temp_result.r = _c_temp.r; temp_result.i = _c_temp.i; };
  { results[i].r = results[i].r + temp_result.r; results[i].i = results[i].i + temp_result.i; };

  temp_charge.r = b->particles[i]->charge;
  temp_charge.i = (real) 0.0;
  { complex _c_temp; _c_temp.r = (temp_vector.r * temp_charge.r) - (temp_vector.i * temp_charge.i); _c_temp.i = (temp_vector.r * temp_charge.i) + (temp_vector.i * temp_charge.r); temp_result.r = _c_temp.r; temp_result.i = _c_temp.i; };
  { results[j].r = results[j].r - temp_result.r; results[j].i = results[j].i - temp_result.i; };
      }
      results[i].i = -results[i].i;
      { (b->particles[i]->field).r = (b->particles[i]->field).r + results[i].r; (b->particles[i]->field).i = (b->particles[i]->field).i + results[i].i; };

   }

   b->cost += (7.0 * 61.4 * b->num_particles * b->num_particles);
}
...
ShiftLocalExp (int my_id, box *pb, box *cb)
{
   ...
   z0_pow_minus_n.i = One.i;
   for (i = 0; i < Expansion_Terms; i++) {
      { pb->local_expansion[i].r = pb->local_expansion[i].r + pb->x_expansion[i].r; pb->local_expansion[i].i = pb->local_expansion[i].i + pb->x_expansion[i].i; };

      { complex _c_temp; _c_temp.r = (z0_pow_n.r * pb->local_expansion[i].r) - (z0_pow_n.i * pb->local_expansion[i].i); _c_temp.i = (z0_pow_n.r * pb->local_expansion[i].i) + (z0_pow_n.i * pb->local_expansion[i].r); temp_exp[i].r = _c_temp.r; temp_exp[i].i = _c_temp.i; };
      { complex _c_temp; _c_temp.r = (z0_pow_n.r * z0.r) - (z0_pow_n.i * z0.i); _c_temp.i = (z0_pow_n.r * z0.i) + (z0_pow_n.i * z0.r); z0_pow_n.r = _c_temp.r; z0_pow_n.i = _c_temp.i; };
   }
   for (i = 0; i < Expansion_Terms; i++) {
      result_exp[i].r = (real) 0.0;
  ...
  { result_exp[i].r = result_exp[i].r + temp.r; result_exp[i].i = result_exp[i].i + temp.i; };
      ...
      { complex _c_temp; _c_temp.r = (temp.r * z0_pow_minus_n.r) - (temp.i * z0_pow_minus_n.i); _c_temp.i = (temp.r * z0_pow_minus_n.i) + (temp.i * z0_pow_minus_n.r); result_exp[i].r = _c_temp.r; result_exp[i].i = _c_temp.i; };
      { complex _c_temp; _c_temp.r = (z0_pow_minus_n.r * z0_inv.r) - (z0_pow_minus_n.i * z0_inv.i); _c_temp.i = (z0_pow_minus_n.r * z0_inv.i) + (z0_pow_minus_n.i * z0_inv.r); z0_pow_minus_n.r = _c_temp.r; z0_pow_minus_n.i = _c_temp.i; };
   }
   {pthread_mutex_lock(&(G_Memory->lock_array)[(cb->exp_lock_index)]);};
   for (i = 0; i < Expansion_Terms; i++) {
      { (cb->local_expansion[i]).r = (cb->local_expansion[i]).r + result_exp[i].r; (cb->local_expansion[i]).i = (cb->local_expansion[i]).i + result_exp[i].i; };
...
EvaluateLocalExp (int my_id, box *b)
{
  ...
  temp.r = (real) j;
  temp.i = (real) 0.0;
  { complex _c_temp; _c_temp.r = (result.r * z0.r) - (result.i * z0.i); _c_temp.i = (result.r * z0.i) + (result.i * z0.r); result.r = _c_temp.r; result.i = _c_temp.i; };
  { complex _c_temp; _c_temp.r = (temp.r * (b->local_expansion[j]).r) - (temp.i * (b->local_expansion[j]).i); _c_temp.i = (temp.r * (b->local_expansion[j]).i) + (temp.i * (b->local_expansion[j]).r); temp.r = _c_temp.r; temp.i = _c_temp.i; };
  { result.r = result.r + temp.r; result.i = result.i + temp.i; };
      }
      { (b->particles[i]->field).r = (b->particles[i]->field).r + result.r; (b->particles[i]->field).i = (b->particles[i]->field).i + result.i; };
