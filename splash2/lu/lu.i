...

{
  ...
  for (i=0;i<P;i++) {
    last_malloc[i] = (double *) malloc(proc_bytes[i] + 4096);
    if (last_malloc[i] == ((void*)0)) {
      fprintf(stderr,"Could not malloc memory blocks for proc %d\n",i);
      exit(-1);
    }
    last_malloc[i] = (double *) (((unsigned) last_malloc[i]) + 4096 -
                     ((unsigned) last_malloc[i]) % 4096);
