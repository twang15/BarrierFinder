...
POINT Ni;
 ...
 pe = hit->pelem;
 ps = (SPHERE *)pe->data;
 { Ni[0] = Pi[0] - ps->center[0]; Ni[1] = Pi[1] - ps->center[1]; Ni[2] = Pi[2] - ps->center[2]; };
...
MATRIX normMat;
 ...
 for (i = 0; i < po->numelements; i++)
  ...
  { rad_vector[0] = surf_point[0] - center_point[0]; rad_vector[1] = surf_point[1] - center_point[1]; rad_vector[2] = surf_point[2] - center_point[2]; };
  { ps->center[0] = center_point[0]; ps->center[1] = center_point[1]; ps->center[2] = center_point[2]; };

  ps->rad = ( sqrt( rad_vector[0]*rad_vector[0] + rad_vector[1]*rad_vector[1] + rad_vector[2]*rad_vector[2] ));
...
IRECORD *hit;
 ...
 { V[0] = ps->center[0] - pr->P[0]; V[1] = ps->center[1] - pr->P[1]; V[2] = ps->center[2] - pr->P[2]; };
 vsq = ( V[0]*V[0] + V[1]*V[1] + V[2]*V[2] );
 b = ( V[0]*pr->D[0] + V[1]*pr->D[1] + V[2]*pr->D[2] );

 if (vsq > ps->rad2 && b < 1e-07)
  return (0);
 ...
 if (t1 > 1e-07)
  {
  { (sphhit)->t = t1; (sphhit)->pelem = pe; };
  sphhit++;
  nhits++;
  }

 { (sphhit)->t = t2; (sphhit)->pelem = pe; };
 nhits++;
...
MATRIX xinvT;
 ...
 for (i = 0; i < numelems; i++)
  ...
  new_rad = ( sqrt( rad_vector[0]*rad_vector[0] + rad_vector[1]*rad_vector[1] + rad_vector[2]*rad_vector[2] ));
