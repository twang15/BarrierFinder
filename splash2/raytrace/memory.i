...
void GlobalHeapWalk()
 ...
 while (curr < endmem)
  ...
  if (curr->cksm != 0x55AA55AA)
   {
   fprintf(stderr, "GlobalHeapWalk: Invalid checksum in node.\n");
   exit(1);
   }

  curr = (NODE *)((U8 *)(curr) + (curr->size + nodesize));
  }
...
UINT size;
 ...
 begmem = gm->freelist;
 endmem = (NODE *)((U8 *)(gm->freelist) + (size));

 gm->freelist->size = size - nodesize;
 gm->freelist->next = ((void*)0);
 gm->freelist->free = 1;
 gm->freelist->cksm = 0x55AA55AA;
...
CHAR *msg;
 ...
 if (curr->size - size > (sizeof(NODE) + 16))
  {
  next = (NODE *)((U8 *)(curr) + (nodesize + size));
  next->size = curr->size - nodesize - size;
  next->next = curr->next;
  next->free = 1;
  next->cksm = 0x55AA55AA;
  curr->size = size;
  }
 else
  next = curr->next;
...
UINT size;
 ...
 pn = (NODE *)((U8 *)(p) + (-nodesize));

 if (pn->cksm != 0x55AA55AA)
  {
 ...
 if (newsize <= oldsize)
  {
  if (oldsize - newsize < (sizeof(NODE) + 16))
   return (p);

  pn->size = newsize;

  next = (NODE *)((U8 *)(p) + (newsize));
  next->size = oldsize - nodesize - newsize;
  next->next = ((void*)0);
  next->free = 0;
  next->cksm = 0x55AA55AA;
  next = (NODE *)((U8 *)(next) + (nodesize));
 ...
 if (next < endmem && next->free && totsize >= newsize)
  ...
  if (totsize - newsize < (sizeof(NODE) + 16))
   {
   ...
   node = (NODE *)((U8 *)(p) + (newsize));
   node->next = next->next;
   node->size = totsize - nodesize - newsize;
   node->free = 1;
   node->cksm = 0x55AA55AA;
...
void *p;
 ...
 ncom = 1;
 next = ((void*)0);
 curr = (NODE *)((U8 *)(pn) + (pn->size + nodesize));
...
INT count;
 ...
 switch (ObjectType)
  ...
  case 0:
   ...
   mem_grid += n;
   maxmem_grid = ( (mem_grid) > (maxmem_grid) ? (mem_grid) : (maxmem_grid) );
   break;
  ...
  case 1:
   ...
   mem_voxel += n;
   maxmem_voxel = ( (mem_voxel) > (maxmem_voxel) ? (mem_voxel) : (maxmem_voxel) );
   break;
  ...
  case 2:
   ...
   mem_hashtable += n;
   maxmem_hashtable = ( (mem_hashtable) > (maxmem_hashtable) ? (mem_hashtable) : (maxmem_hashtable) );
   }
   break;
  ...
  case 3:
   ...
   mem_emptycells += n;
   maxmem_emptycells = ( (mem_emptycells) > (maxmem_emptycells) ? (mem_emptycells) : (maxmem_emptycells) );
   }
   break;
  ...
  case 6:
   ...
   mem_bintree += n;
   maxmem_bintree = ( (mem_bintree) > (maxmem_bintree) ? (mem_bintree) : (maxmem_bintree) );
   break;
  ...
  case 7:
   ...
   mem_pepArray += n;
   maxmem_pepArray = ( (mem_pepArray) > (maxmem_pepArray) ? (mem_pepArray) : (maxmem_pepArray) );
   break;
