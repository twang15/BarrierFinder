...
REAL x, y;
 ...
 if (View.projection == 0)
  ...
  VecNorm(dir);
  { ray->D[0] = dir[0]; ray->D[1] = dir[1]; ray->D[2] = dir[2]; };
  { ray->P[0] = View.eye[0]; ray->P[1] = View.eye[1]; ray->P[2] = View.eye[2]; };
  }
 else
  {
  ...
  { ray->P[0] = origin[0]; ray->P[1] = origin[1]; ray->P[2] = origin[2]; };
  }
...
INT pid;
 ...
 while (GetJobs(&job, pid) != 0)
  ...
  while (GetRayJobFromBundle(&job, &x, &y))
   ...
   while (PopRayTreeStack(ray, pid) != 0)
    {
    ...
    if (hit)
     ...
     { Ipoint[0] = ray->P[0] + ray->D[0]*hitrecord.t; Ipoint[1] = ray->P[1] + ray->D[1]*hitrecord.t; Ipoint[2] = ray->P[2] + ray->D[2]*hitrecord.t; };


     (*po->procs->normal)(&hitrecord, Ipoint, N);


     if ((( ray->D[0]*N[0] + ray->D[1]*N[1] + ray->D[2]*N[2] )) > 0.0)
     ...
     { c[0] = View.bkg[0]; c[1] = View.bkg[1]; c[2] = View.bkg[2]; };
     { c[0] = ray->weight * c[0]; c[1] = ray->weight * c[1]; c[2] = ray->weight * c[2]; };
