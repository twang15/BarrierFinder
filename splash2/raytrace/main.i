...
int atomic_cmpxchg(int oldcount, int newcount)
{
  ...
  if(oldcount == temp)
  {
    (gm->start).count = newcount;
    //noScheduling();
    updatedcount = oldcount;// updatedcount == oldcount, update succeeds.
  }
  else
  {
    updatedcount = (gm->start).count; // updatedcount != oldcount, update fails.
    //noScheduling();
  }
  return updatedcount;
}
...
# 260 "main.c"
 {
    ...
    int lsense = (gm->start).gsense;
    while (1) {
 int oldcount = (gm->start).count;
 ...
 int updatedcount, retVal;
  ...
  if (updatedcount == oldcount) {
     if ((newcount == (gm->nprocs) && lsense == 1) || (newcount == 0 && lsense == -1))
      {
        (gm->start).gsense = -lsense;
        Criteria(1);
      }
     break;
 }
    }
    while ((gm->start).gsense == lsense)
 ;
    Criteria(1);
...
# 430 "main.c"
 ...
 {pthread_mutex_init(&(gm->pidlock), ((void*)0));}
 {pthread_mutex_init(&(gm->ridlock), ((void*)0));}
 {pthread_mutex_init(&(gm->memlock), ((void*)0));}
 {int i; for (i = 0; i < (nprocs); i++)
    pthread_mutex_init(&((gm->wplock)[i]), ((void*)0));}
 ...
 for (i = 0; i < gm->nprocs - 1; i++)
  {
pthread_t t;
pthread_create(&t, ((void*)0), (StartRayTrace), ((void*)0));
}
