...
POINT I;
 ...
 I_dot_N = ( I[0]*N[0] + I[1]*N[1] + I[2]*N[2] );
 I_dot_N = ( (I_dot_N) > 0.0 ? (I_dot_N) : -(I_dot_N) );
 I_dot_N = 1.0/I_dot_N;

 { vprime[0] = I_dot_N * I[0]; vprime[1] = I_dot_N * I[1]; vprime[2] = I_dot_N * I[2]; };
 { N2[0] = 2.0 * N[0]; N2[1] = 2.0 * N[1]; N2[2] = 2.0 * N[2]; };

 { R[0] = vprime[0] + N2[0]; R[1] = vprime[1] + N2[1]; R[2] = vprime[2] + N2[2]; };
 VecNorm(R);
 }
# 96 "shade.c"
...
REAL kn;
 ...
 { vprime[0] = I_dot_N * I[0]; vprime[1] = I_dot_N * I[1]; vprime[2] = I_dot_N * I[2]; };
 { vplusn[0] = vprime[0] + N[0]; vplusn[1] = vprime[1] + N[1]; vplusn[2] = vprime[2] + N[2]; };

 vprime_sq = ( vprime[0]*vprime[0] + vprime[1]*vprime[1] + vprime[2]*vprime[2] );
 vplusn_sq = ( vplusn[0]*vplusn[0] + vplusn[1]*vplusn[1] + vplusn[2]*vplusn[2] );
 ...
 if (kf > 1e-07)
  ...
  { vplusn[0] = kf * vplusn[0]; vplusn[1] = kf * vplusn[1]; vplusn[2] = kf * vplusn[2]; };
  { T[0] = vplusn[0] - N[0]; T[1] = vplusn[1] - N[1]; T[2] = vplusn[2] - N[2]; };
  VecNorm(T);
  }
 else
  return (0);
...
INT pid;
 ...
 po = hit->pelem->parent;
 s = po->surf;
 { surfcol[0] = s->fcolor[0]; surfcol[1] = s->fcolor[1]; surfcol[2] = s->fcolor[2]; };
 ...
 { shad_ray.P[0] = iP[0]; shad_ray.P[1] = iP[1]; shad_ray.P[2] = iP[2]; };
 { Evec[0] = -ray->D[0]; Evec[1] = -ray->D[1]; Evec[2] = -ray->D[2]; };
 ...
 for (i = 0; i < nlights; i++)
  {
  { Lvec[0] = lptr->pos[0] - iP[0]; Lvec[1] = lptr->pos[1] - iP[1]; Lvec[2] = lptr->pos[2] - iP[2]; };
  lightlen = ( sqrt( Lvec[0]*Lvec[0] + Lvec[1]*Lvec[1] + Lvec[2]*Lvec[2] ));
  VecNorm(Lvec);
  { shad_ray.D[0] = Lvec[0]; shad_ray.D[1] = Lvec[1]; shad_ray.D[2] = Lvec[2]; };

  {pthread_mutex_lock(&(gm->ridlock));};
  shad_ray.id = gm->rid++;
  {pthread_mutex_unlock(&(gm->ridlock));};

  NdotL = ( N[0]*Lvec[0] + N[1]*Lvec[1] + N[2]*Lvec[2] );
  ...
  if (NdotL > 0.0)
   ...
   if (shadtrans > 0.0)
    ...
    if (s->kspec > 0.0)
     {
     { Hvec[0] = Lvec[0] + Evec[0]; Hvec[1] = Lvec[1] + Evec[1]; Hvec[2] = Lvec[2] + Evec[2]; };
     VecNorm(Hvec);
     NdotH = ( N[0]*Hvec[0] + N[1]*Hvec[1] + N[2]*Hvec[2] );
 ...
 { col[0] = ray->weight * col[0]; col[1] = ray->weight * col[1]; col[2] = ray->weight * col[2]; };
 ...
 if ((ray->level) + 1 < Display.maxlevel)
  {
  { secondary_ray.P[0] = iP[0]; secondary_ray.P[1] = iP[1]; secondary_ray.P[2] = iP[2]; };
