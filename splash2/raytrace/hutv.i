...
RAY *r;
 ...
 while (lookup_emptycells(indx, gr) == 1)
  {
  indx = next_voxel(r);
...
INT *status;
 ...
 while (v->celltype == 5 || v->celltype == 3)
  {
  if (v->celltype == 5)
   {
   send_ray(r, v);
  ...
  if (lookup_emptycells(indx, rinfo->grid) != 1)
   {
   v = lookup_hashtable(indx, rinfo->grid);
   if (v->celltype != 5 && v->celltype != 3)
    {
   ...
   {
   v = next_nonempty_leaf(r, 1, status);
...
GRID *g;
 ...
 ri->delta[0] = r->D[0] == 0.0 ? 1e+32 : g->cellsize[0] / ( (r->D[0]) > 0.0 ? (r->D[0]) : -(r->D[0]) );
 ri->delta[1] = r->D[1] == 0.0 ? 1e+32 : g->cellsize[1] / ( (r->D[1]) > 0.0 ? (r->D[1]) : -(r->D[1]) );
 ri->delta[2] = r->D[2] == 0.0 ? 1e+32 : g->cellsize[2] / ( (r->D[2]) > 0.0 ? (r->D[2]) : -(r->D[2]) );
 ...
 if ((v = next_nonempty_leaf(r, 0, &status)) != ((void*)0))
  {
  ri = r->ri;
  indx = ri->index1D;
  gr = ri->grid;
