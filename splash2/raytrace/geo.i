 ...
 *totalElements = 0;
 ...
 po = gm->modelroot;
 npepa = ObjectMalloc(7, *totalElements);
...
MATRIX modelInvT;
 ...
 if (! (TraversalType == 0 && ModelNorm == 0) )
  {


  norm_minx = norm_miny = norm_minz = 1e+32;
  norm_maxx = norm_maxy = norm_maxz = -1e+32;

  while (po)
   {
   norm_minx = ( (norm_minx) < (po->bv.dnear[0]) ? (norm_minx) : (po->bv.dnear[0]) );
   norm_miny = ( (norm_miny) < (po->bv.dnear[1]) ? (norm_miny) : (po->bv.dnear[1]) );
   norm_minz = ( (norm_minz) < (po->bv.dnear[2]) ? (norm_minz) : (po->bv.dnear[2]) );
   norm_maxx = ( (norm_maxx) > (po->bv.dfar[0]) ? (norm_maxx) : (po->bv.dfar[0]) );
   norm_maxy = ( (norm_maxy) > (po->bv.dfar[1]) ? (norm_maxy) : (po->bv.dfar[1]) );
   norm_maxz = ( (norm_maxz) > (po->bv.dfar[2]) ? (norm_maxz) : (po->bv.dfar[2]) );
  ...
  scale_min = ( (norm_minx) < (norm_miny) ? (norm_minx) : (norm_miny) );
  scale_min = ( (scale_min) < (norm_minz) ? (scale_min) : (norm_minz) );
  scale_max = ( (norm_maxx) > (norm_maxy) ? (norm_maxx) : (norm_maxy) );
  scale_max = ( (scale_max) > (norm_maxz) ? (scale_max) : (norm_maxz) );
...
CHAR *GeoFileName;
 ...
 while ((stat = fscanf(pf, "%s %s", objstr, objname)) != (-1))
  {
