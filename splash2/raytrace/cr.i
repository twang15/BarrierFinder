...
void init_masks()
 ...
 for (i = 0; i < n; i++)
  {
  empty_masks[i] = ((((UINT)1) << (sizeof(UINT)*8 - 1)) >> i);
...
GRID *g;
 ...
 if (btn->n[imax] > 1)
  ...
  btn1 = ObjectMalloc(6, 1);
  btn2 = ObjectMalloc(6, 1);
...
INT num_prims;
 ...
 ht = ObjectMalloc(2, hu_numbuckets);
 ng->hashtable = ht;

 ncells = (R64)(hu_gridsize * hu_gridsize * hu_gridsize);
 total_cells += ncells;

 ec = ObjectMalloc(3, (INT)ncells);
 ng->emptycells = ec;
