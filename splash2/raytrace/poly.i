...
POINT Ni;
 ...
 pe = hit->pelem;
 pp = (POLY *)pe->data;
 { Ni[0] = pp->norm[0]; Ni[1] = pp->norm[1]; Ni[2] = pp->norm[2]; };
 }
# 283 "poly.c"
...
IRECORD *hit;
 ...
 Rd_dot_Pn = ( pp->norm[0]*pr->D[0] + pp->norm[1]*pr->D[1] + pp->norm[2]*pr->D[2] );

 if (( (Rd_dot_Pn) > 0.0 ? (Rd_dot_Pn) : -(Rd_dot_Pn) ) < 1e-07)
  return (0);

 Ro_dot_Pn = ( pp->norm[0]*pr->P[0] + pp->norm[1]*pr->P[1] + pp->norm[2]*pr->P[2] );

 tval = -(pp->d + Ro_dot_Pn)/Rd_dot_Pn;
 if (tval < 1e-07)
  return (0);

 { I[0] = pr->P[0] + pr->D[0]*tval; I[1] = pr->P[1] + pr->D[1]*tval; I[2] = pr->P[2] + pr->D[2]*tval; };
...
MATRIX xinvT;
 ...
 for (i = 0; i < numelems; i++)
  ...
  norm[0] = ( (pp->norm[0]) > 0.0 ? (pp->norm[0]) : -(pp->norm[0]) );
  norm[1] = ( (pp->norm[1]) > 0.0 ? (pp->norm[1]) : -(pp->norm[1]) );
  norm[2] = ( (pp->norm[2]) > 0.0 ? (pp->norm[2]) : -(pp->norm[2]) );
...
FILE *pf;
 ...
 for (i = 0; i < po->numelements; i++)
  ...
  if (!pnormals)
   {
   vp = vptr + (*vindex);
   { pnorm[0] = 0.0; pnorm[1] = 0.0; pnorm[2] = 0.0; };
   ...
   for (j = 0; j < pp->nverts - 2; j++)
    ...
    { tmppnt[0] = (*vptmp)[0] - (*vp)[0]; tmppnt[1] = (*vptmp)[1] - (*vp)[1]; tmppnt[2] = (*vptmp)[2] - (*vp)[2]; };
    { tmppnt2[0] = (*vptmp2)[0] - (*vptmp)[0]; tmppnt2[1] = (*vptmp2)[1] - (*vptmp)[1]; tmppnt2[2] = (*vptmp2)[2] - (*vptmp)[2]; };

    { cross[0] = tmppnt[1]*tmppnt2[2] - tmppnt[2]*tmppnt2[1]; cross[1] = tmppnt[2]*tmppnt2[0] - tmppnt[0]*tmppnt2[2]; cross[2] = tmppnt[0]*tmppnt2[1] - tmppnt[1]*tmppnt2[0]; };
    { pnorm[0] = pnorm[0] + cross[0]; pnorm[1] = pnorm[1] + cross[1]; pnorm[2] = pnorm[2] + cross[2]; };

    vp = vptmp;
    vindex += 1;
    }

   { tmppnt[0] = (*vptmp2)[0] - (*vp)[0]; tmppnt[1] = (*vptmp2)[1] - (*vp)[1]; tmppnt[2] = (*vptmp2)[2] - (*vp)[2]; };
   vindex = pp->vindex;
   vp = vptr + (*vindex);

   { tmppnt2[0] = (*vp)[0] - (*vptmp2)[0]; tmppnt2[1] = (*vp)[1] - (*vptmp2)[1]; tmppnt2[2] = (*vp)[2] - (*vptmp2)[2]; };
   { cross[0] = tmppnt[1]*tmppnt2[2] - tmppnt[2]*tmppnt2[1]; cross[1] = tmppnt[2]*tmppnt2[0] - tmppnt[0]*tmppnt2[2]; cross[2] = tmppnt[0]*tmppnt2[1] - tmppnt[1]*tmppnt2[0]; };
   { pnorm[0] = pnorm[0] + cross[0]; pnorm[1] = pnorm[1] + cross[1]; pnorm[2] = pnorm[2] + cross[2]; };

   vp = vptr + (*vindex);
   { tmppnt[0] = (*vp)[0] - (*vptmp2)[0]; tmppnt[1] = (*vp)[1] - (*vptmp2)[1]; tmppnt[2] = (*vp)[2] - (*vptmp2)[2]; };
   vptmp = vptr + (*(vindex + 1));

   { tmppnt2[0] = (*vptmp)[0] - (*vp)[0]; tmppnt2[1] = (*vptmp)[1] - (*vp)[1]; tmppnt2[2] = (*vptmp)[2] - (*vp)[2]; };
   { cross[0] = tmppnt[1]*tmppnt2[2] - tmppnt[2]*tmppnt2[1]; cross[1] = tmppnt[2]*tmppnt2[0] - tmppnt[0]*tmppnt2[2]; cross[2] = tmppnt[0]*tmppnt2[1] - tmppnt[1]*tmppnt2[0]; };
   { pnorm[0] = pnorm[0] + cross[0]; pnorm[1] = pnorm[1] + cross[1]; pnorm[2] = pnorm[2] + cross[2]; };
   { pp->norm[0] = 1.0/( sqrt( pnorm[0]*pnorm[0] + pnorm[1]*pnorm[1] + pnorm[2]*pnorm[2] )) * pnorm[0]; pp->norm[1] = 1.0/( sqrt( pnorm[0]*pnorm[0] + pnorm[1]*pnorm[1] + pnorm[2]*pnorm[2] )) * pnorm[1]; pp->norm[2] = 1.0/( sqrt( pnorm[0]*pnorm[0] + pnorm[1]*pnorm[1] + pnorm[2]*pnorm[2] )) * pnorm[2]; };
   }
