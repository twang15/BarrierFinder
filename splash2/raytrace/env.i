...
void PrintEnv()
 ...
 printf(View.projection == 0 ? "perspective\n" : "orthographic\n");
...
REAL x, y, z;
 ...
 if (( (rx) > 0.0 ? (rx) : -(rx) ) < 1e-07)
  {
  MatrixIdentity(M);
  Rotate(1, M, 1.570796327 * (( (y) > 0.0 ? 1.0 : -1.0 )));
  return;
  }
...
CHAR *s;
 ...
 for (i = 0; i < (sizeof(cmdtab)/sizeof(CTE)); i++)
