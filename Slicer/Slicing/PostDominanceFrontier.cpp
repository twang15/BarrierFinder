...
template class InverseDominanceFrontierBase<BasicBlock>;
}
...
#elif 0
INITIALIZE_PASS(CreateHammockCFG, 
    "Creates Hammock Graph from Control Flow Graph",
    "create-hammock-cfg", 
    true,
    false
...
#else
INITIALIZE_PASS_BEGIN(CreateHammockCFG, "create-hammock-cfg",
    "Creates Hammock Graph from Control Flow Graph", true, false)
INITIALIZE_PASS_DEPENDENCY(LoopInfoWrapperPass)
INITIALIZE_PASS_END(CreateHammockCFG, "create-hammock-cfg",
    "Creates Hammock Graph from Control Flow Graph", true, false)
#endif
...
#if 1
INITIALIZE_PASS_BEGIN(PostDominanceFrontier, "postdom-frontier",
                "Post Dominance Frontier Construction", true, true)
//INITIALIZE_PASS_DEPENDENCY(CreateHammockCFG)
INITIALIZE_PASS_DEPENDENCY(PostDominatorTree)
//INITIALIZE_PASS_DEPENDENCY(UnifyFunctionExitNodes)
INITIALIZE_PASS_END(PostDominanceFrontier, "postdom-frontier",
                "Post Dominance Frontier Construction", true, true)

PostDominanceFrontier::PostDominanceFrontier()
  : FunctionPass(ID), Base()
{
...
bool PostDominanceFrontier::runOnFunction(Function & F)
{
  releaseMemory();
  //getAnalysis<CreateHammockCFG>();
  //getAnalysis<UnifyFunctionExitNodes>();
  PostDominatorTree &DT = getAnalysis<PostDominatorTree>();

  //Base->Roots = DT.getRoots();
