...
namespace llvm { namespace mods {
  ...
  void dumpFunModifies(const Modifies::ModSet &m, const Function *fun)
  {
      errs() << fun->getName() << "\n";
  ...
  void dumpModifies(const ProgramStructure &P, Modifies &MOD)
  {
    errs() << "\n==== MODSET DUMP ====\n";
      ...
      Modifies &MOD) 
  {
          ...
          c != f->second.end(); ++c)
      {
        ...
        else if (c->getType() == CMD_DREF_VAR) 
        {
          ...
          for (PTSet::const_iterator p = S.begin(); p != S.end(); ++p)
            if (!isLocalToFunction(p->first, f->first) && !isConstantValue(p->first))
    ...
    for (Callgraph::const_iterator i = CG.begin_closure(); i != CG.end_closure(); ++i) 
    {
