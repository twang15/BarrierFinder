...
namespace llvm {
  ...
  const FunctionType *getCalleePrototype(const CallInst *C) 
  {
    ...
    else if (const PointerType *ptrType = dyn_cast<PointerType>(callie->getType()))
  ...
  {
    if (llvm::Function const* const F = llvm::dyn_cast<llvm::Function const>(V))
  ...
  {
    if (llvm::Instruction const* I = llvm::dyn_cast<llvm::Instruction const>(V))
  ...
  const Value *elimConstExpr(const Value *V) 
  {
    if (const ConstantExpr *CE = dyn_cast<ConstantExpr>(V)) 
    {
      if (Instruction::isBinaryOp(CE->getOpcode()))
        return V;
      assert((CE->getOpcode() == llvm::Instruction::GetElementPtr || CE->isCast()) &&
