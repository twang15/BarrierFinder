...
namespace llvm { namespace ptr { namespace detail {
  ...
  template<typename OutIterator> void toRuleCode(const Value *V, OutIterator out) 
  {
    if (const llvm::Instruction *I = llvm::dyn_cast<llvm::Instruction>(V)) 
    {
      ...
      else if (const llvm::CallInst *C = llvm::dyn_cast<llvm::CallInst>(I)) 
      {
        ...
        else if (isMemoryCopy(C->getCalledValue()) || isMemoryMove(C->getCalledValue())) 
        {
      ...
      else if (const llvm::ExtractValueInst *EV = llvm::dyn_cast<llvm::ExtractValueInst>(I)) 
      {
      ...
      else if (const llvm::InsertValueInst *IV = llvm::dyn_cast<llvm::InsertValueInst>(I)) 
      {
      ...
      else if (const llvm::IntToPtrInst *ITPI = llvm::dyn_cast<llvm::IntToPtrInst>(I)) 
      {
        errs() << __func__ << ": WARNING[PointsTo]: Integer ";

        if (const llvm::ConstantInt *C = llvm::dyn_cast<llvm::ConstantInt>(ITPI->getOperand(0)))
          errs() << "(" << C->getValue() << ") ";
      ...
      else if (const llvm::SelectInst *SEL = llvm::dyn_cast<llvm::SelectInst>(I)) 
      {
    ...
    else if (const llvm::GlobalVariable *GV = llvm::dyn_cast<llvm::GlobalVariable>(V)) 
    {
