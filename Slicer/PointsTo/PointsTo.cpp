...
namespace llvm { namespace ptr { namespace detail {
  ...
  template <typename OutIterator>
    void CallMaps::collectCallRuleCodes(const CallInst *c, const Function *f, OutIterator out) 
    {
  ...
  bool CallMaps::compatibleFunTypes(const FunctionType *f1, const FunctionType *f2) 
  {
...
namespace llvm { namespace ptr {
  ...
  static bool applyRule(PointsToSets &S, ASSIGNMENT<VARIABLE<const llvm::Value *>,
  ...
  static bool applyRules(const RuleCode &RC, PointsToSets &S, const llvm::DataLayout &DL)
  {
      ...
      const PointsToSets &S, const int idx) 
  {
    ...
    if (it == S.end()) 
    {
      ...
      //disable unexpected output when debugging.
#if 0
  ...
  //whether memLoc points to a heap variable.
  //Assumption: if any one of the heap variable is pointed by memLoc, return true.
  bool pointsToHeapVariable(const llvm::Value *const &memLoc, const PointsToSets &S, const int idx)
  {
    const PointsToSets::const_iterator it = S.find(Ptr(memLoc, idx));
    if (it == S.end())
    {
    ...
    const PTSet &ptSet = it->second;
    PTSet::iterator I = ptSet.begin(), E = ptSet.end();
    for( ; I != E; ++I)
    {    
      Pointee pointee = *I;
      if(const CallInst *call = dyn_cast<CallInst>(pointee.first))
      {
        if(call->getCalledFunction()->getName() == StringRef("malloc"))
        {
          return true;
        }
      }
  ...
  void dumpPointsToSet(const PTSet &ptSet)
  {
    PTSet::iterator I = ptSet.begin(), E = ptSet.end();
    for( ; I != E; ++I)
    {
      Pointee pointee = *I;
      if(const Instruction *val = dyn_cast<Instruction>(I->first))
      {
          errs() << "\tFUN=" << val->getParent()->getParent()->getName() <<
            " OFF=" << I->second << " ";
          val->dump();
          //pointee.first->print(errs()); is equivalent to val->dump().
      }
      else if(const GlobalVariable *gv = dyn_cast<GlobalVariable>(I->first))
      {
