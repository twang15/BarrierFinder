...
# 1 "/home/tao/tools/llvm.debug/bin/../lib/clang/3.7.0/include/stddef.h" 1 3 4
# 62 "/home/tao/tools/llvm.debug/bin/../lib/clang/3.7.0/include/stddef.h" 3 4
...
# 1 "/home/tao/tools/llvm.debug/bin/../lib/clang/3.7.0/include/stdarg.h" 1 3 4
# 30 "/home/tao/tools/llvm.debug/bin/../lib/clang/3.7.0/include/stdarg.h" 3 4
typedef __builtin_va_list va_list;
# 50 "/home/tao/tools/llvm.debug/bin/../lib/clang/3.7.0/include/stdarg.h" 3 4
...

{
  ...
  if (rowsperproc * rootN * 2 * sizeof(double) >= 4096) {
    pages = (2 * pad_length * sizeof(double) * rowsperproc) / 4096;
    if (pages * 4096 != 2 * pad_length * sizeof(double) * rowsperproc) {
      pages ++;
    }
    pad_length = (pages * 4096) / (2 * sizeof(double) * rowsperproc);
  } else {
    pad_length = (4096 - (rowsperproc * rootN * 2 * sizeof(double))) /

      (2 * sizeof(double) * rowsperproc);
    if (pad_length * (2 * sizeof(double) * rowsperproc) !=
        (4096 - (rowsperproc * rootN * 2 * sizeof(double)))) {
  ...
  x = (double *) malloc(2*(N+rootN*pad_length)*sizeof(double)+4096);;
  trans = (double *) malloc(2*(N+rootN*pad_length)*sizeof(double)+4096);;
  umain = (double *) malloc(2*rootN*sizeof(double));;
  umain2 = (double *) malloc(2*(N+rootN*pad_length)*sizeof(double)+4096);;
  ...
  if (Global == ((void*)0)) {
    ...
    exit(-1);
  } else if (trans == ((void*)0)) {
    printerr("Could not malloc memory for trans\n");
    exit(-1);
  } else if (umain == ((void*)0)) {
    printerr("Could not malloc memory for umain\n");
    exit(-1);
  } else if (umain2 == ((void*)0)) {
    printerr("Could not malloc memory for umain2\n");
    exit(-1);
  }

  x = (double *) (((unsigned) x) + 4096 - ((unsigned) x) % 4096);
  trans = (double *) (((unsigned) trans) + 4096 - ((unsigned) trans) % 4096);
  umain2 = (double *) (((unsigned) umain2) + 4096 - ((unsigned) umain2) % 4096);
# 299 "fft.c"
  printf("\n");
  ...
  {pthread_mutex_init(&(Global->idlock), ((void*)0));};
  Global->id = 0;
  InitX(N, x);
  ...
  InitU(N,umain);
  InitU2(N,umain2,rootN); printf("main %d\n", pthread_self());
  ...
  for (i=1; i<P; i++) {
    {
      pthread_t t;
      pthread_create(&t, ((void*)0), (SlaveStart), ((void*)0));
...

{
  ...
  for (q=0; 1<<q<N; q++) {
    ...
    for (j=0; j<n1; j++) {
      ...
      u[2*(base+j)] = cos(2.0*3.1416*j/(2*n1));
      u[2*(base+j)+1] = -sin(2.0*3.1416*j/(2*n1));
    }
  }
}
...

{
  ...
  for (j=0; j<n1; j++) {
    k = j*(rootN+pad_length);
    for (i=0; i<n1; i++) {
      u[2*(k+i)] = cos(2.0*3.1416*i*j/(N));
      u[2*(k+i)+1] = -sin(2.0*3.1416*i*j/(N));
    }
  }
}
...

{
  ...
  for (k=0; k<N; k++) {
    j = BitReverse(M, k);
    if (j > k) {
      {double tmp; tmp=x[2*j]; x[2*j]=x[2*k]; x[2*k]=tmp;};
      {double tmp; tmp=x[2*j+1]; x[2*j+1]=x[2*k+1]; x[2*k+1]=tmp;};
    }
  }
}
