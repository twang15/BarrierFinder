...
unsigned int  ExecutionState::stateCounter = 0;
#endif
...
ExecutionState::ExecutionState(Executor *_executor, KFunction *kf)
  ...
  bInvariantState = false;
  bRecording = false;
  bEnumerating = true;
  bDumpTrace = true;
  bDirectDescendent = false;
  bOutSideSyncRegion = false;
  bRoot = false;
  currentSyncRegionID = -1;
  bFirstTimeExplored = false;

#ifdef __STANDALONE_DEBUG__  
  UINT_parent_id = stateCounter;
  stateCounter++;
  UINT_state_id = stateCounter;
  UINT_fork_id = stateCounter-1;
#endif
...
void ExecutionState::notifyAll(wlist_id_t wlist, uint64_t all) {
  assert(wlist > 0);

  //notifyAll is invoked by pthread_exit
  if(all == 2)
  {
    numOfThreads--;
  }
  ...
  if (wl.size() > 0) {
    for (std::set<thread_uid_t>::iterator it = wl.begin(); it != wl.end(); it++) {
      ...
      //if (current child thread is the last children thread) && 
      //the notification is sent to main thread
      if((thread.tuid.first == 0) && (numOfThreads != 1))
      {
        continue;
      }
...
ExecutionState *ExecutionState::branch(bool copy) {
  ...
  falseState->bRecording = false;
  falseState->bEnumerating = true;
  falseState->bDumpTrace = true;
  falseState->bDirectDescendent = false;
  falseState->bFirstTimeExplored = false;
  falseState->bRoot = false;
  falseState->bOutSideSyncRegion = false;

  if(this->bRoot)
  {
    falseState->bInvariantState = true;
    falseState->bRoot = true;
  }
  else
  {  
    if(this->bInvariantState && this->bDirectDescendent)
    {
      falseState->bInvariantState = true;
    }
    else if(this->bInvariantState && !this->bDirectDescendent)
    {
      falseState->bInvariantState = false;
    }
    else if(!this->bInvariantState && this->bOutSideSyncRegion)
    {
      falseState->bInvariantState = false;
      falseState->bEnumerating = this->bEnumerating;
      this->bOutSideSyncRegion = false;
    }
    else if(!this->bInvariantState)
    {
      falseState->bInvariantState = false;
    }
  }
...
#ifdef __STANDALONE_DEBUG__
    //falseState->UINT_fork_id and falseState->UINT_state_id 
    //has already been set when falseState object is created. 
    falseState->UINT_parent_id = UINT_state_id;
    stateCounter++;
    falseState->UINT_state_id = stateCounter;
    falseState->UINT_fork_id = stateCounter - 1;
#endif    
  }
...
StackTrace ExecutionState::getStackTrace(ExecutionState::threads_ty::iterator& it) const 
{
  StackTrace result;
  Thread & thread = it->second;
  stack_ty & stack = thread.stack;
...
ExecutionContext::CallStack ExecutionState::getExecutionContext(const Thread &thread) const
{
  //current thread context
  ExecutionContext::CallStack callStack;
  const stack_ty & stack = thread.stack;
       ...
       it != ie; ++it) 
  {
    ...
    const StackFrame &sf = *it;
    ExecutionContext::Position position = make_pair(sf.kf, target);
    callStack.push_back(position);
...
ExecutionContext ExecutionState::getExecutionContext() const
{
  ExecutionContext context;

  //current thread context
  const Thread &thread = crtThread();
  context.curCallStack = getExecutionContext(thread);

  //other thread contexts
  for(ExecutionState::threads_ty::const_iterator it = threads.begin();
      it != threads.end(); it++)
  {
    //Exclude current running thread, because it has already been recorded above.
    if(&thread != &(it->second))
    {
      context.threadCallStacks[it->second.getTid()] = getExecutionContext(it->second);
    }
  }
...
/*
 * For debugging. dump all threads' stack traces.
 * */
void ExecutionState::dumpStacks() 
{
  for(ExecutionState::threads_ty::iterator it = threads.begin();
      it != threads.end();  it++)
  {
    //get the stack for current thread
    StackTrace stackTrace = getStackTrace(it);
    if(&(crtThread()) == &(it->second))
    {
      std::cout << "-->> Current Thread\n";
    }
    std::cout << "\t" << &(it->second) << "\t===Thread ID: " << it->second.tuid.first << "\t";
    std::cout << ", mystring: " << ((char*)getGlobalVariable()) << "\t";
//    std::cout << ", counter0: " << *((int*)getGlobalVariable("counter0")) << "\t";
//    std::cout << ", counter1: " << *((int*)getGlobalVariable("counter1")) << "\t";
//    std::cout << ", gSyncVar_0: " << *((int*)getGlobalVariable("gSyncVar_0")) << "\t";
//    std::cout << ", gSyncVar_1: " << *((int*)getGlobalVariable("gSyncVar_1")) << "\t";
    std::cout << "\n pc: \n";
    it->second.pc->inst->dump();
    it->second.pc->inst->getParent()->dump();
