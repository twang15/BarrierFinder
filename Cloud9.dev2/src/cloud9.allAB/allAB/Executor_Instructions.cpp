...
{
  KInstruction *ki = activeState->pc();
  Instruction *currentInst = ki->inst;
  Function *currentFunc = currentInst->getParent()->getParent();
  KFunction *kFun = kmodule->functionMap[currentFunc];
  StackFrame &sf = activeState->stack().back();
  std::ostream &os = std::cout; 
  
  for (llvm::Function::iterator bbit = currentFunc->begin(), 
      bbie = currentFunc->end(); bbit != bbie; ++bbit) 
  {
    for (llvm::BasicBlock::iterator it = bbit->begin(), ie = bbit->end();
        it != ie; ++it)
    {
      currentInst = it;
      if(AllocaInst *alloc = dyn_cast<AllocaInst>(currentInst))
      {
        currentInst->dump();
        //MemoryObject allocated for local variables.
        os << "\tStack variable at: ";

        KInstruction *ki = kFun->instructionMap[currentInst];
        if(!(getDestCell(*activeState, ki).value.isNull()))
        {
          if(ConstantExpr *CE = dyn_cast<ConstantExpr>(getDestCell(*activeState, ki).value))
          {
            activeState->addressSpace().DumpMemoryObject(CE->getZExtValue());
          }
        }
      }
    }
      ...
      bbie = currentFunc->end(); bbit != bbie; ++bbit) 
  {
        ...
        it != ie; ++it)
    {
      ...
      if(AllocaInst *alloc = dyn_cast<AllocaInst>(currentInst))
      {
        ...
        if(!(getDestCell(*activeState, ki).value.isNull()))
        {
          getDestCell(*activeState, ki).value->dump(false);
        }
      }
      else
      {
        unsigned numOperands = currentInst->getNumOperands();
        for (unsigned j=0; j<numOperands; j++) 
        {
          Value *v = currentInst->getOperand(j);

          if (Instruction *inst = dyn_cast<Instruction>(v)) 
          {
            int vnumber = kFun->registerMap[inst];
            os << "\tRegister: ";

            if(!sf.locals[vnumber].value.isNull())
            {
              sf.locals[vnumber].value->dump(false);
            }
          } 
          else if (Argument *a = dyn_cast<Argument>(v)) 
          {
            int vnumber = a->getArgNo();
            os << "\tArgument: ";
          ...
          else 
          {
            ...
            int vnumber = -(kmodule->getConstantID(c, ki) + 2);
            vnumber = -(vnumber+2);
            os << "\tConstant: ";

            if(!(kmodule->constantTable[vnumber].value.isNull()))
            {
              kmodule->constantTable[vnumber].value->dump(false);
            }
          }
        }
      }
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  //This is a short-cut to implement a front-end function.
  //Usually, it should follow the example of handleCheckInvariant, etc.  
  //Here, it is a hack.
    ...
    || funcName == "printChar")
  {
    ...
    if(!state.bDumpTrace)
    {
      //specialization of case Instruction::Ret
