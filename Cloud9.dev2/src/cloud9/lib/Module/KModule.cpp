...
void KModule::prepare(const Interpreter::ModuleOptions &opts,
  ...
  for (std::map<std::string, Function*>::iterator it = fnList.begin();
    ...
    //The following maps:
    // <Function, KFunction> ---> implicit
    // <KFunction, Function> --->explicit
    // <KFunction, KInstruction> ---> explicit
    // <KInstruction, Instruction> --> explicit
    //
    // But, if we want to see the value for each Instruction and its operands,
    // we need <Instruction, KInstruction> --- > usually very easy.
    // we also need <KInstruction, KFunction> --> This information does not exist now.
    // With proper management of these mapping, we can achieve the following effect:
    // a. Given an Instruction, we can see its operands values.
    // b. Given an Instruction, we can see which instructions and BasicBlocks have been executed
    //    in the current function and what are their results.
    // c. Given an Instruction, we can know its KFunction, KInstruction and Function(very easy)
    //
    // With these information, the debugging facility for interpretee is equivalent to gdb's local
    // variable inspection function, but is even more powerful. Because, it not only contains the
    // information of each instruction's temporary result, but also contains the instruction traces.
    // The temporary results are not destoried as in gdb, and traces is also maintained. These information
    // is lack in gdb, but can be added if necessary to facilitate general debugging.
    // 
    // In gdb, if we want to see the result of a program point that has been passed, we have to resort to
    // reverse execution or just set a breakpoint at the point and re-execute. The reverse execution can
    // also be implemented in Cloud9, but with little meaning just as gdb's reverse execution. The execution
    // trace facility here may be ported back to gdb, but not necessary. The trace debugging, if implemented,
    // would ease the debugging effort of interpretee. But, we may also set a breakpoint at arbitrary point
    // in the function, except the still live variables/registers cannot be easily inspected. Another
    // potential challenge here is how to management instructions in loop ? The current mechanism is to
    // keep the latest value as Cloud9 writes to the same register Cell for the same instruction.
    //
    // The implementation of such facilities requires careful consideration of related mapping and 
    // data structures. After clear design, the implementation would be fairly easy.
...
KFunction::KFunction(llvm::Function *_function,
  ...
  //This map can be computed in a debugging function as necessary or
  //just promote it to be a member of KFunction.
//  std::map<Instruction*, unsigned> registerMap;
