...
namespace klee {
  ...
  public:
    ...
    /// Find a global variable's concrete address(physical address)
    /// If found, return the base address; otherwise, return NULL.
    void * findGlobalVariable(StringRef &gVarName);
    ...
    void DumpMemoryObject(uint64_t address);
    void DumpMemoryObject(const MemoryObject *mo);
