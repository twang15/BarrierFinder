...
void * AddressSpace::findGlobalVariable(StringRef &gVarName)
{
  for (MemoryMap::iterator it = objects.begin(), ie = objects.end();
      it != ie; ++it) 
  {
    const MemoryObject *mo = it->first;
    if(mo->isGlobal && mo->allocSite != NULL)
    {
      if(const GlobalVariable *gv = dyn_cast<GlobalVariable>(mo->allocSite))
      {
        //gv is global output string we are looking for.
        if(gv->getName() == gVarName)
        {
          //Retrieve its value.
...
void AddressSpace::DumpMemoryObject(uint64_t address)
{
  MemoryObject hack(address);
  DumpMemoryObject(&hack);
}

void AddressSpace::DumpMemoryObject(const MemoryObject* mobject)
{
  if (const MemoryMap::value_type *res = objects.lookup_previous(mobject))
  {
    const MemoryObject *mo = res->first;
    const ObjectState *ostate = res->second;
    if ((mo->size==0 && mobject->address==mo->address) ||
        (mobject->address - mo->address < mo->size)) 
    {
      DumpMemoryObject(std::cout, mo, ostate, false);
    }
  }
...
void AddressSpace::DumpMemoryObject(std::ostream &os, const MemoryObject *mo,
  ...
  //if we have the type information(int, char, int *, etc), we can dump its concrete content.
  //The type information can be obtained in two ways:
  //a. when we build a MemoryObject, we embed the type information along with other information
  //   such as size; when we dump the content, we can extract this information from MemoryObject
  //   and use it to help decide the format of the concreteStore.
  //b. We can extract the type information inside this function based on the allocSite information.
  //   The AllocSite information can be obtained inside getAllocInfo function and passed back here.
  //   It contains type information.
  //c. Do it one-by-one manually.
  //
  //It is reasonable to implement option b. It does not has the overhead of a but does bring the benefit
  //of a; it also reduces the effort for c.
  //
  //The reference code to dump concreteStore is in: CopyOutConcretes. But, we do not copy concreteStore into
  //MemoryObject's address (mmaped real memory, shared by all ExecutionState), we output it on the screen
  //directly.
  //
  //Besides, it is also interesting if we dump the corresponding assembly line information for
  //eacho allocSite. The MemoryObject constructor has to be modified to collect such information.
  ...
  if(!mo->isUserSpecified)
  {
    ...
    //read out mo->size content of ostate->concreteStore
    if(mo->allocSite)
    {
      if(const Instruction *i=dyn_cast<Instruction>(mo->allocSite))
      {
        //type checking.
      }
      else if(const GlobalValue *gv = dyn_cast<GlobalValue>(mo->allocSite))
      {
        //type checking.
      }
      else
...
/*For debugging in gdb*/
void AddressSpace::DumpContents() const
{
  DumpContents(std::cout);
}
...
void AddressSpace::DumpContents(std::ostream &os) const {
  ...
  for (MemoryMap::iterator it = objects.begin(), ie = objects.end();
    ...
    const MemoryObject *mo = it->first;
    const ObjectState *ostate = it->second;
    DumpMemoryObject(os, mo, ostate, false);
  }
}
